class DesktopJS {
    ...
    newWindow(obj) {
        if (typeof obj !== "object") {
            ...
            if (obj.properties.resizable !== undefined) {
                if (obj.properties.resizable !== "none") {
            ...
            let title;
            if (obj.properties.title !== undefined) {
              title = obj.title;
            } else {
              title = "Window title";
            }
            if (obj.properties.openCenter === true) {
              win.classList.add("openCenter");
            }
            win.innerHTML = `
                <div class="BTNs">
                    ...
                    <div style="display: inline-flex;justify-content: flex-end;width: calc(100% - 75px);height: 20px;margin: 0;">
                      <span>${title}</span>
                    </div>
                </div>
                <div class="inner">
    ...
    makeDraggable(id) {
      if (document.querySelector(".el-" + id) !== undefined) {
        let elementSelector = ".el-" + id;
        var dragStartX, dragStartY;
        var objInitLeft, objInitTop;
        var inDrag = false;
        var dragTarget = document.querySelector(".el-" + id);
